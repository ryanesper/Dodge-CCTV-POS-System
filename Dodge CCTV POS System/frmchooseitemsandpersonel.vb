...
Public Class frmchooseitemsandpersonel
    ...
    Dim nupvalue As Integer
    Dim quantitytoremove As Integer

    Private Sub frmchooseitemsandpersonel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        lviassignpersonel.Columns.Clear()
        lviassignpersonel.Columns.Add("Assigned Personel", 370, HorizontalAlignment.Left)
        ...
        'loadproductname()
        'loadotherproduct()
        If loaduseditemsinoperation = True Then
            loaditemsinoperation()
            loadoperationassignedpersonel()
            ...
            btnok.Enabled = True
        ElseIf loaduseditemsinoperation = False Then

            btnselectfromstocks.Enabled = True
            btnremove.Enabled = True
            btnrestock.Enabled = True
            btnok.Text = "Ok"
            cbopersonel.Enabled = True
        End If


    End Sub

    Public Sub loadpersonel()

        openconnection()
        cbopersonel.Items.Clear()
        Dim ds As New DataSet
        Dim sql As String

        sql = "select fullname from tbl_personel"
        sqlda = New MySqlDataAdapter(sql, con)

        sqlda.Fill(ds, "fms")
        For Each r As DataRow In ds.Tables(0).Rows
            cbopersonel.Items.Add(r("fullname"))
        Next
        con.Close()

        For Each item As ListViewItem In lviassignpersonel.Items
            If lviassignpersonel.Items.Count > 0 Then
                cbopersonel.Items.Remove(item.SubItems(0).Text)
            End If
        Next

    End Sub

    Public Sub loaditemsinoperation()
        ...
        While (reader.Read())
            ...
            ElseIf unitprice < 1000 Then
                item.SubItems.Add("Php " & unitprice)
            End If
            ...
            ElseIf sellingprice < 1000 Then
                item.SubItems.Add("Php " & sellingprice)
            End If
            ...
            ElseIf totalprice < 1000 Then
                item.SubItems.Add("Php " & totalprice)
            End If
        ...
        ElseIf grandtotal > 0 And grandtotal < 1000 Then
            txttotalprice.Text = "Php " & grandtotal
    ...
    Private Sub loadoperationassignedpersonel()
        openconnection()
        lviassignpersonel.Items.Clear()
        cmd = New MySqlCommand("select * from tbl_operation_assigned_personel where operation_id = '" & txtoprno.Text & "'", con)
        reader = cmd.ExecuteReader
        While (reader.Read())
            Dim item As New ListViewItem(reader("fullname").ToString())
            lviassignpersonel.Items.Add(item)
        End While
        reader.Close()

        For Each fullname As ListViewItem In lviassignpersonel.Items
            cbopersonel.Items.Remove(fullname.SubItems(0).Text)
        Next
        con.Close()
    End Sub
    ...
    Private Sub lviitems_KeyDown(sender As Object, e As KeyEventArgs) Handles lviitems.KeyDown
    ...
    Private Sub cbopersonel_KeyDown(sender As Object, e As KeyEventArgs) Handles cbopersonel.KeyDown

        If e.KeyCode = Keys.Enter Then
            openconnection()
            If cbopersonel.Text <> "" Then
                cmd.CommandText = "Select * from tbl_personel where fullname = '" & cbopersonel.Text & "'"
                reader = cmd.ExecuteReader
                If reader.HasRows = False Then
                    reader.Close()
                    cmd = New MySqlCommand("INSERT INTO `tbl_personel`(`fullname`) VALUES ('" & cbopersonel.Text & "')", con)
                    cmd.ExecuteNonQuery()
                End If
                reader.Close()
            End If

            Dim personel1 As New ListViewItem(cbopersonel.Text)
            lviassignpersonel.Items.Add(personel1)
            cbopersonel.Text = ""
            loadpersonel()
            con.Close()

            frmmain.lviassignpersonel.Items.Clear()
            For Each personel As ListViewItem In lviassignpersonel.Items
                frmmain.lviassignpersonel.Items.Add(personel.SubItems(0).Text)
            Next
    ...
    Private Sub lviassignpersonel_KeyDown(sender As Object, e As KeyEventArgs) Handles lviassignpersonel.KeyDown

        If btnok.Text <> "Edit" Then
            If e.KeyCode = Keys.Delete Then
                For Each personel As ListViewItem In lviassignpersonel.Items
                    If lviassignpersonel.SelectedItems.Count > 0 Then
                        personel = lviassignpersonel.SelectedItems.Item(0)
                        cbopersonel.Items.Add(personel.SubItems(0).Text)
                        personel.Remove()
                    End If
                Next
            End If
    ...
    Private Sub btnselectfromstocks_Click(sender As Object, e As EventArgs) Handles btnselectfromstocks.Click
        ...
        processtooperationortransaction = "operation"
    ...
    Private Sub updateassignedpersonel()
        openconnection()
        cmddelete = "DELETE FROM `tbl_operation_assigned_personel` where operation_id='" & txtoprno.Text & "'"
        sqlda = New MySqlDataAdapter(cmddelete, con)
        ds = New DataSet()
        sqlda.Fill(ds)
        For Each assignedpersonel As ListViewItem In lviassignpersonel.Items
    ...
    Private Sub btncancel_Click(sender As Object, e As EventArgs) Handles btncancel.Click
        ...
        lvirestock.Items.Clear()
        lvispecs.Items.Clear()
        loadassignedpersonelanditemusedinmainifoperationidexist()
        Me.Close()

    End Sub

    Private Sub loadassignedpersonelanditemusedinmainifoperationidexist()
        openconnection()
        cmd.CommandText = "select operation_id from tbl_operation_item_used where operation_id = '" & txtoprno.Text & "'"
        reader = cmd.ExecuteReader
        If reader.HasRows Then
            reader.Close()
            frmmain.lviassignpersonel.Items.Clear()
            ...
            While (reader.Read())
                ...
                frmmain.lviassignpersonel.Items.Add(personel)
            End While
            reader.Close()
            ...
            While (reader.Read())
                ...
                frmmain.lviitems.Items.Add(items)
            End While
            reader.Close()
        End If
        con.Close()
    ...
    Private Sub btnothers_Click(sender As Object, e As EventArgs) Handles btnothers.Click
        ...
        productmode = "Add Others Mode"
        frmproduct.lblproduct.Text = "Add Others"
